...
- Fixed exports configuration for @datawarp/mastra-evals package (#d641d91)
...
- Implemented tsup bundling for @datawarp/mastra-mcp (#8468b7f)
...
- Deprecated `@datawarp/mastra-tts` in favor of new Mastra speech providers (#cfb966f)
...
# @datawarp/mastra-speech-google v0.1.0 (2025-02-17)
...
This release introduces a more robust and maintainable architecture for Google Speech integration while improving overall type safety and build processes. Users should migrate from `@datawarp/mastra-tts` to this new provider-specific module for Google Speech services.
...
- Deprecated `@datawarp/mastra-tts` in favor of new speech provider modules (#cfb966f)
...
- Updated to @datawarp/mastra-core v0.2.0 with numerous improvements
- Removed unnecessary Babel dependencies
- Updated Node.js type definitions
...
- Migration required from `@datawarp/mastra-tts` to new speech provider system
...
This release marks a significant milestone with the introduction of dedicated speech provider modules and improved build system architecture. The deprecation of `@datawarp/mastra-tts` represents our move toward more specialized provider-specific implementations.
...
This release marks the first stable version of the PlayAI speech provider, bringing improved type safety, better error handling, and more robust build tooling. The module now uses the new Mastra speech provider architecture, replacing the deprecated `@datawarp/mastra-tts` package.
...
Here's the changelog for the @datawarp/mastra-pg package for the week of February 10-17, 2025:

## @datawarp/mastra-pg v0.1.0

### Major Changes

- Combined PostgreSQL packages into a single unified `@datawarp/mastra-pg` package (#c87eb4e)
...
- Relocated package from `@datawarp/mastra-vector-astra` to `@datawarp/mastra-astra` with improved organization (#c87eb4e)
...
Note: This release includes numerous dependency updates from the core package (@datawarp/mastra-core@0.2.0) that improve overall stability and functionality.
...
- Relocated package from `@datawarp/mastra-vector-chroma` to `@datawarp/mastra-chroma` with improved organization and structure (#684d5d1)
...
- Relocated package from `@datawarp/mastra-vector-pinecone` to `@datawarp/mastra-pinecone` with reorganized source structure (#c87eb4e)
...
- Relocated package from `@datawarp/mastra-vector-qdrant` to `@datawarp/mastra-qdrant` with improved organization (c87eb4e)
...
# @datawarp/mastra-upstash v0.1.0

## Major Changes

- Combined all Upstash-related packages into a single `@datawarp/mastra-upstash` package for better organization and maintenance (c87eb4e)
...
- Renamed package from `@datawarp/mastra-vector-vectorize` to `@datawarp/mastra-vectorize` - update your imports accordingly (#91c81f5)
...
Note: This release includes a significant number of dependency updates from the core package (@datawarp/mastra-core) which provide underlying improvements to the vectorize module's functionality.
