...
- Updated dependencies for @datawarp/mastra-deployer and @datawarp/mastra-core
...
- Updated dependencies to @datawarp/mastra-core@0.5.0-alpha.9
...
Based on the git diff provided, I'll create a changelog for the Mastra AI evals package. However, I notice that most changes are dependency updates to @datawarp/mastra-core rather than direct changes to the evals package itself.

# Changelog for Mastra AI Evals (v0.1.8-alpha.9)

## Build/Deployment Improvements

- Updated @datawarp/mastra-core dependency to version 0.5.0-alpha.9 through multiple alpha releases

Note: This release primarily consists of internal dependency updates. No direct feature changes, improvements, or bug fixes were made to the evals package itself during this period. All changes were related to keeping the package in sync with the latest @datawarp/mastra-core updates.

For details about the underlying core changes, please refer to the @datawarp/mastra-core changelog.
...
- Updated @datawarp/mastra-core dependency to version 0.5.0-alpha.9
...
- Updated to @datawarp/mastra-core v0.5.0-alpha.9
- Updated to @datawarp/mastra-deployer v0.1.8-alpha.9
...
- Updated @datawarp/mastra-core to version 0.5.0-alpha.9
- Updated @datawarp/mastra-deployer to version 0.1.8-alpha.9
...
Note: This release includes several dependency updates to @datawarp/mastra-core (v0.5.0-alpha.9) and @datawarp/mastra-deployer (v0.1.8-alpha.9) but no other significant changes to the Vercel deployer functionality itself.
...
- Updated dependency on @datawarp/mastra-core to version 0.5.0-alpha.9 and its subsequent alpha releases
...
For details about specific changes in the core module that may affect Azure speech functionality, please refer to the @datawarp/mastra-core v0.5.0 changelog.
...
- Updated to latest @datawarp/mastra-core dependency (v0.5.0-alpha.9)

### Important Notes

- This module is now deprecated - users should migrate to `@datawarp/mastra-voice-deepgram` for future development
...
- This module is now deprecated - please use `@datawarp/mastra-voice-elevenlabs` instead for future development
...
For users of this module, we recommend planning migration to `@datawarp/mastra-voice-elevenlabs` for continued feature development and support.
...
- Updated @datawarp/mastra-core dependency to version 0.5.0-alpha.9 through multiple incremental releases
...
For details about the underlying core changes that this update enables, please refer to the [@datawarp/mastra-core changelog](https://github.com/mastra/core/blob/main/CHANGELOG.md).
...
- This module is now deprecated - users should migrate to `@datawarp/mastra-voice-murf` for future updates and improvements
...
For users currently using `@datawarp/mastra-speech-murf`, we recommend planning migration to `@datawarp/mastra-voice-murf` to ensure continued support and access to new features.
...
- This module is now deprecated. Please migrate to `@datawarp/mastra-voice-openai` for future development.

The changes in this release are focused on maintaining compatibility with the core Mastra AI framework while the module transitions to its new home at `@datawarp/mastra-voice-openai`. No breaking changes were introduced in this update.

For users of this module, we recommend:

1. Planning migration to `@datawarp/mastra-voice-openai`
...
Based on the provided git diff, this appears to be primarily a series of version bumps and dependency updates for the @datawarp/mastra-speech-playai module. Here's the structured changelog:

# Mastra Speech PlayAI Changelog (March 3-10, 2025)

## Build & Dependencies

- Updated @datawarp/mastra-core dependency to version 0.5.0-alpha.9 (#e9fbac5, #1e8bcbc, #aeb5e36, #f2301de)
- Released version 0.1.7-alpha.9 with latest core updates

## Important Notes

⚠️ This module is now deprecated. Users should migrate to `@datawarp/mastra-voice-playai` for future development.
...
Based on the git diff provided, I'll create a changelog for the speech/replicate module. However, I notice that most changes are dependency updates to @datawarp/mastra-core without direct changes to the speech/replicate module itself. Here's an appropriate changelog:
...
For details about the core changes affecting this module, please refer to the [@datawarp/mastra-core changelog](link-to-core-changelog).
...
This module is marked as DEPRECATED - users should migrate to @datawarp/mastra-voice-speechify instead.

---

Note: This changelog period primarily consisted of internal dependency updates. No direct feature changes, bug fixes, or performance improvements were made to the Speechify module itself. Users should begin planning migration to the new @datawarp/mastra-voice-speechify package as this module is deprecated.

## stores/pg

Based on the provided git diff, this appears to be primarily a version bump and dependency update changelog for the @datawarp/mastra-pg package. Here's the structured changelog:

# @datawarp/mastra-pg v0.1.8-alpha.9 Changelog (March 3-10, 2025)

## Build/Deployment Improvements

- Updated package version from 0.1.7 to 0.1.8-alpha.9
- Updated dependency on @datawarp/mastra-core to version 0.5.0-alpha.9
...
The changes appear to be part of a larger alpha release cycle, with multiple alpha versions (0-9) being published to track updates in the core dependency. For specific feature changes, users should refer to the @datawarp/mastra-core v0.5.0-alpha changelog.
...
- Upgraded to @datawarp/mastra-core@0.5.0-alpha.9
...
Based on the git diff provided, I'll create a changelog for the Pinecone store module. However, I notice that most changes are dependency updates to @datawarp/mastra-core without direct changes to the Pinecone module itself. Here's the appropriate changelog:
...
Note: This release primarily contains internal dependency updates without direct functional changes to the Pinecone store module. For details about the core module changes, please refer to the @datawarp/mastra-core changelog.
...
- Updated to latest @datawarp/mastra-core dependency (v0.5.0-alpha.9) for enhanced stability and features
...
The changes shown are primarily version bumps and dependency updates to @datawarp/mastra-core. Since the actual code changes are not visible in the diff, I've kept the changelog focused on the version progression and core dependency updates. If you have additional specific changes that should be highlighted, please let me know.

## stores/vectorize

Based on the provided git diff, I'll create a changelog for the Mastra AI Vectorize module. However, I notice that most changes are dependency updates to @datawarp/mastra-core rather than direct changes to the Vectorize module itself.
