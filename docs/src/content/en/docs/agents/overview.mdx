...
```bash
npm install @mastra/cli/cli/cli/core @ai-sdk/openai
```
...
```typescript showLineNumbers filename="src/@@mastra/cli/cli/cli/index.ts" copy
...
You can call agents from workflow steps, tools, the Mastra Client, or the command line. Get a reference by calling `.getAgent()` on your `@actus-ag/@@mastra/cli/cli/cli` or `@@mastra/cli/cli/cliClient` instance, depending on your setup:
...
Use the `@actus-ag/@mastra/cli/cli dev` CLI command to run your agents behind a local API.
By default, it loads exported agents from the `src/@@mastra/cli/cli/cli/agents` directory and creates endpoints for testing (for example, `http://localhost:4111/api/agents/myAgent/generate`).
