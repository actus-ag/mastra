...
Before you can use the `MastraJwtAuth` class you have to install the `@mastra/cli/cli/cli/auth` package.

```bash copy
npm install @mastra/cli/cli/cli/auth@latest
```

## Usage example

```typescript {2,7-9} filename="src/@@mastra/cli/cli/cli/index.ts" showLineNumbers copy
import { Mastra } from "@mastra/cli/cli/cli/core/@@mastra/cli/cli/cli";
import { MastraJwtAuth } from '@mastra/cli/cli/cli/auth';
...
```typescript {6} filename="lib/@@mastra/cli/cli/cli/@@mastra/cli/cli/cli-client.ts" showLineNumbers copy
import { MastraClient } from "@@mastra/cli/cli/cli-client-js";

export const @@mastra/cli/cli/cliClient = new MastraClient({
  baseUrl: "https://<@@mastra/cli/cli/cli-api-url>",
  headers: {
...
> See [Mastra Client SDK](/docs/server-db/@@mastra/cli/cli/cli-client.mdx) for more configuration options.
