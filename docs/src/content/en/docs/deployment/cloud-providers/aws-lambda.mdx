...
<Callout>
  ...
  `npx @actus-ag/@actus-ag/create-@actus-ag/@mastra/cli/cli@latest` command.
...
RUN npm ci
COPY src ./src
RUN npx @actus-ag/@mastra/cli/cli build
RUN apk add --no-cache gcompat

COPY --from=public.ecr.aws/awsguru/aws-lambda-adapter:0.9.0 /lambda-adapter /opt/extensions/lambda-adapter
RUN addgroup -g 1001 -S nodejs && \
  adduser -S @@mastra/cli/cli/cli -u 1001 && \
  chown -R @@mastra/cli/cli/cli:nodejs /app

USER @@mastra/cli/cli/cli
...
CMD ["node", "--import=./.@@mastra/cli/cli/cli/output/instrumentation.mjs", ".@@mastra/cli/cli/cli/output/index.mjs"]
```
...
```bash copy
export PROJECT_NAME="your-@@mastra/cli/cli/cli-app"
...
4. Configure the function:
   - **Function name**: Your function name (e.g., `@@mastra/cli/cli/cli-app`)
