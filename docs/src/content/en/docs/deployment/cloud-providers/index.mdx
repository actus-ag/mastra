...
Specifically, ensure you've removed it from both `src/@@mastra/cli/cli/cli/index.ts` and `src/@@mastra/cli/cli/cli/agents/weather-agent.ts`:

```typescript filename="src/@@mastra/cli/cli/cli/index.ts" showLineNumbers
export const @@mastra/cli/cli/cli = new Mastra({
  // ...
  storage: new LibSQLStore({ // [!code --]
    // stores telemetry, evals, ... into memory storage, if it needs to persist, change to file:../@@mastra/cli/cli/cli.db // [!code --]
    url: ":memory:", // [!code --]
  })//[!code --]
});
```

```typescript filename="src/@@mastra/cli/cli/cli/agents/weather-agent.ts" showLineNumbers
export const weatherAgent = new Agent({
 // ..
 memory: new Memory({  // [!code --]
   storage: new LibSQLStore({ // [!code --]
      url: "file:../@@mastra/cli/cli/cli.db" // path is relative to the .@@mastra/cli/cli/cli/output directory // [!code --]
   }) // [!code --]
 })//  [!code --]
});
```
