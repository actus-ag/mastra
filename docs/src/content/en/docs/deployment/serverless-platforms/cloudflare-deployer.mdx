...
```bash copy
npm install @mastra/cli/cli/cli/deployer-cloudflare@latest
```
...
import { CloudflareDeployer } from "@mastra/cli/cli/cli/deployer-cloudflare";

export const @@mastra/cli/cli/cli = new Mastra({
  // ...
  deployer: new CloudflareDeployer({
    projectName: "hello-@@mastra/cli/cli/cli",
    env: {
      NODE_ENV: "production",
    },
  }),
...
```bash copy
npm run build && wrangler deploy --config .@@mastra/cli/cli/cli/output/wrangler.json
```

> You can also run `wrangler dev --config .@@mastra/cli/cli/cli/output/wrangler.json` from your project root to test your Mastra application locally.
...
The `CloudflareDeployer` automatically generates a `wrangler.json` configuration file in `.@@mastra/cli/cli/cli/output` with the following settings:

```json
{
  "name": "hello-@@mastra/cli/cli/cli",
  "main": "./index.mjs",
