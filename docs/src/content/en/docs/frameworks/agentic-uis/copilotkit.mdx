...
<Tabs items={["npm", "yarn", "pnpm"]}>
  <Tabs.Tab>
    ```bash copy
    npm install @copilotkit/runtime @ag-ui/@@mastra/cli/cli/cli
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```bash copy
    yarn add @copilotkit/runtime @ag-ui/@@mastra/cli/cli/cli
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```bash copy
    pnpm add @copilotkit/runtime @ag-ui/@@mastra/cli/cli/cli
    ```
  </Tabs.Tab>
</Tabs>
...
```typescript filename="src/@@mastra/cli/cli/cli/index.ts" showLineNumbers copy {5-8,12-28}
import { Mastra } from "@mastra/cli/cli/cli/core/@@mastra/cli/cli/cli";
import { registerCopilotKit } from "@ag-ui/@@mastra/cli/cli/cli";
...
<Tabs items={["npm", "yarn", "pnpm"]}>
  <Tabs.Tab>
    ```bash copy
    npm install @copilotkit/react-core @copilotkit/react-ui @copilotkit/runtime @ag-ui/@@mastra/cli/cli/cli
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```bash copy
    yarn add @copilotkit/react-core @copilotkit/react-ui @copilotkit/runtime @ag-ui/@@mastra/cli/cli/cli
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```bash copy
    pnpm add @copilotkit/react-core @copilotkit/react-ui @copilotkit/runtime @ag-ui/@@mastra/cli/cli/cli
    ```
  </Tabs.Tab>
</Tabs>
...
import { MastraAgent } from "@ag-ui/@@mastra/cli/cli/cli";
import { NextRequest } from "next/server";

export const POST = async (req: NextRequest) => {
  const @@mastra/cli/cli/cliAgents = MastraAgent.getLocalAgents({
    @@mastra/cli/cli/cli,
    agentId: "weatherAgent",
  });

  const runtime = new CopilotRuntime({
    agents: @@mastra/cli/cli/cliAgents,
  });
...
<Tabs items={["npm", "yarn", "pnpm"]}>
  <Tabs.Tab>
    ```bash copy
    npm install @actus-ag/@@mastra/cli/cli/client-js
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```bash copy
    yarn add @actus-ag/@@mastra/cli/cli/client-js
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```bash copy
     pnpm add @actus-ag/@@mastra/cli/cli/client-js
    ```
  </Tabs.Tab>
</Tabs>
...
export const POST = async (req: NextRequest) => {
  ...
  const @@mastra/cli/cli/cliClient = new MastraClient({ baseUrl });

  const @@mastra/cli/cli/cliAgents = await MastraAgent.getRemoteAgents({ @@mastra/cli/cli/cliClient });
