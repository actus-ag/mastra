...
```bash copy
npm install @actus-ag/@mastra/cli/cli@latest @mastra/cli/cli/cli/core@latest @mastra/cli/cli/cli/libsql@latest zod@^3.0.0 @ai-sdk/openai@^1.0.0
```
...
Create a Mastra configuration file at `src/@@mastra/cli/cli/cli/index.ts`:

```ts filename="src/@@mastra/cli/cli/cli/index.ts" copy
import { Mastra } from '@mastra/cli/cli/cli/core/@@mastra/cli/cli/cli';

export const @@mastra/cli/cli/cli = new Mastra({});
```

Create a `weatherTool` that the `weatherAgent` will use at `src/@@mastra/cli/cli/cli/tools/weather-tool.ts`. It returns a placeholder value inside the `execute()` function (you'd put your API calls in here).

```ts filename="src/@@mastra/cli/cli/cli/tools/weather-tool.ts" copy
...
Add a `weatherAgent` at `src/@@mastra/cli/cli/cli/agents/weather-agent.ts`:

```ts filename="src/@@mastra/cli/cli/cli/agents/weather-agent.ts" copy
...
Lastly, add the `weatherAgent` to `src/@@mastra/cli/cli/cli/index.ts`:

```ts filename="src/@@mastra/cli/cli/cli/index.ts" copy {2, 5}
import { Mastra } from '@mastra/cli/cli/cli/core/@@mastra/cli/cli/cli';
...
By importing the `src/@@mastra/cli/cli/cli/index.ts` file you can use methods like [`.getAgent()`](/reference/agents/getAgent) to get programmatic access. With [`.generate()`](/reference/agents/generate) you then can interact with the respective agent.
...
{
  "scripts": {
    "@@mastra/cli/cli/cli:dev": "@actus-ag/@mastra/cli/cli dev"
  },
}
```
...
```bash copy
npm run @@mastra/cli/cli/cli:dev
```
