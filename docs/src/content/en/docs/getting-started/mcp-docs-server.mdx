...
`@mastra/cli/cli/cli/mcp-docs-server` provides direct access to Mastra's complete knowledge base in Cursor, Windsurf, Cline, or any other IDE that supports MCP.
...
```json
{
  "mcpServers": {
    "@actus-ag/@@mastra/cli/cli/cli": {
      "command": "npx",
      "args": ["-y", "@mastra/cli/cli/cli/mcp-docs-server"]
    }
  }
}
```
...
```json
{
  "servers": {
    "@actus-ag/@@mastra/cli/cli/cli": {
      "command": "cmd",
      "args": ["/c", "npx", "-y", "@mastra/cli/cli/cli/mcp-docs-server"],
      "type": "stdio"
    }
  }
}
...
Track updates for Mastra and `@mastra/cli/cli/cli/*` packages:
