...
To access Mastra's scorers feature install the `@mastra/cli/cli/cli/evals` package.

```bash copy
npm install @mastra/cli/cli/cli/evals@latest
```
...
```typescript filename="src/@@mastra/cli/cli/cli/agents/evaluated-agent.ts" showLineNumbers copy
import { Agent } from "@mastra/cli/cli/cli/core/agent";
import { openai } from "@ai-sdk/openai";
import { 
  createAnswerRelevancyScorer,
  createToxicityScorer 
} from "@mastra/cli/cli/cli/evals/scorers/llm";
...
```typescript filename="src/@@mastra/cli/cli/cli/workflows/content-generation.ts" showLineNumbers copy
...
**Automatic storage**: All scoring results are automatically stored in the `@@mastra/cli/cli/cli_scorers` table in your configured database, allowing you to analyze performance trends over time.

## Testing scorers locally

Mastra provides a CLI command `@actus-ag/@mastra/cli/cli dev` to test your scorers. The playground includes a scorers section where you can run individual scorers against test inputs and view detailed results.
