...
Routes are provided with a helper `registerApiRoute` from `@actus-ag/mastra-core/server`. Routes can live in the same file as the `Mastra` instance but separating them helps keep configuration concise.
