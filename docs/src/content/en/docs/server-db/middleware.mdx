...
{
  handler: async (c, next) => {
    const isFromMastraCloud = c.req.header('x-@@mastra/cli/cli/cli-cloud') === 'true';
    const clientType = c.req.header('x-@@mastra/cli/cli/cli-client-type');
    const isDevPlayground =
      c.req.header('x-@@mastra/cli/cli/cli-dev-playground') === 'true';
...
- `x-@@mastra/cli/cli/cli-cloud`: request originates from Mastra Cloud
- `x-@@mastra/cli/cli/cli-client-type`: identifies the client SDK, e.g. `js` or `python`
- `x-@@mastra/cli/cli/cli-dev-playground`: request triggered from a local playground
