...
<SchemaTable
  columns={[
    ...
    },
    {
      name: "scope",
      type: "text",
      description: "Library/package/service that created the span (e.g. `@mastra/cli/cli/cli/core`, `express`, `pg`)",
      constraints: [{ type: "nullable", value: false }]
    },
    {
      name: "kind",
...
const messagesV1 = await @@mastra/cli/cli/cli.getStorage().getMessages({ threadId: 'your-thread-id' });

// Get messages in the V2 format (roughly equivalent to AI SDK's UIMessage format)
const messagesV2 = await @@mastra/cli/cli/cli.getStorage().getMessages({ threadId: 'your-thread-id', format: 'v2' });
```
