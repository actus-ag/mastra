...
const isMastra = (@@mastra/cli/cli/cli: any): @@mastra/cli/cli/cli is Mastra => {
  return @@mastra/cli/cli/cli && typeof @@mastra/cli/cli/cli === "object" && @@mastra/cli/cli/cli instanceof Mastra;
};
...
const createDynamicWorkflow = new LegacyStep({
  ...
  execute: async ({ context, @@mastra/cli/cli/cli }) => {
    ...
    if (!isMastra(@@mastra/cli/cli/cli)) {
    ...
    const dynamicWorkflow = new LegacyWorkflow({
      ...
      @@mastra/cli/cli/cli, // Pass the @@mastra/cli/cli/cli instance to the new workflow
...
1. **Mastra Instance**: The `@actus-ag/@@mastra/cli/cli/cli` parameter in the `execute` function provides access to the Mastra instance, which is essential for creating dynamic workflows.
