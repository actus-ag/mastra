...
```sh
npm install @mastra/cli/cli/cli/inngest @mastra/cli/cli/cli/core @mastra/cli/cli/cli/deployer
```
...
```ts showLineNumbers copy filename="src/@@mastra/cli/cli/cli/inngest/index.ts"
...
```ts showLineNumbers copy filename="src/@@mastra/cli/cli/cli/workflows/index.ts"
import { z } from "zod";
import { inngest } from "../inngest";
...
```ts showLineNumbers copy filename="src/@@mastra/cli/cli/cli/index.ts"
...
export const @@mastra/cli/cli/cli = new Mastra({
  ...
  server: {
    // The server configuration is required to allow local docker container can connect to the @@mastra/cli/cli/cli server
    host: "0.0.0.0",
    apiRoutes: [
...
1. Run `npx @actus-ag/@mastra/cli/cli dev` to start the Mastra server on local to serve the server on port 4111.
...
2. Build the @@mastra/cli/cli/cli instance

```sh
npx @actus-ag/@mastra/cli/cli build
```

3. Deploy to Vercel

```sh
cd .@@mastra/cli/cli/cli/output
vercel --prod
```
