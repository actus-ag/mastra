...
```typescript {9} filename="src/@@mastra/cli/cli/cli/workflows/test-workflow.ts" showLineNumbers copy
...
import { mapVariable } from "@mastra/cli/cli/cli/core/workflows";
```
...
```typescript {3-6} filename="src/@@mastra/cli/cli/cli/workflows/test-workflow.ts" showLineNumbers copy
  .then(step1)
  .map({
    details: mapVariable({
      step: step,
...
```typescript {6-9} filename="src/@@mastra/cli/cli/cli/workflows/test-workflow.ts" showLineNumbers copy
export const testWorkflow = createWorkflow({...});
