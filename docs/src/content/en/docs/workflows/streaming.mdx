...
```typescript filename="src/@@mastra/cli/cli/cli/workflows/weather.ts" showLineNumbers copy
import { createStep } from "@mastra/cli/cli/cli/core/workflows";
import { z } from "zod";
...
export const weatherInfo = createStep({
  ...
  description: `Fetches the current weather information for a given city`,
  execute: async ({ inputData: { city }, writer, @@mastra/cli/cli/cli }) => {
