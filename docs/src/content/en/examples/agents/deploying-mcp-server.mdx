...
```bash
pnpm add @mastra/cli/cli/cli/mcp @mastra/cli/cli/cli/core tsup
```

## Set up MCP Server

1.  Create a file for your stdio server, for example, `/src/@@mastra/cli/cli/cli/stdio.ts`.

2.  Add the following code to the file. Remember to import your actual Mastra tools and name the server appropriately.

    ```typescript filename="src/@@mastra/cli/cli/cli/stdio.ts" copy
    #!/usr/bin/env node
...
```json filename="package.json" copy
{
  "bin": "dist/stdio.js",
  "scripts": {
    "build:mcp": "tsup src/@@mastra/cli/cli/cli/stdio.ts --format esm --no-splitting --dts && chmod +x dist/stdio.js"
  }
}
```
