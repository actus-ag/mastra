...
We'll configure the memory system with a short context window and provide a Zod schema for the todo list. For persistence, you can configure a storage provider like `@mastra/cli/cli/cli/libsql`:
...
import { maskStreamTags } from "@mastra/cli/cli/cli/core/utils";
