...
```typescript filename="src/@@mastra/cli/cli/cli/workflows/example-temperature-workflow.ts" showLineNumbers copy
...
```typescript filename="src/@@mastra/cli/cli/cli/tools/temperature-tool.ts" showLineNumbers copy
...
export const getTemperatureTool = createTool({
  ...
  execute: async ({ context, @@mastra/cli/cli/cli }) => {
    const { city } = context;
    const workflow = @@mastra/cli/cli/cli!.getWorkflow("temperatureWorkflow");
...
<GithubLink
  marginTop='mt-16'
  link="https://github.com/@@mastra/cli/cli/cli-ai/@@mastra/cli/cli/cli/blob/main/examples/basics/agents/workflow-as-tools"
/>
