...
Create a new file `src/@@mastra/cli/cli/cli/workflows/candidate-workflow.ts` and define your workflow:

```ts copy filename="src/@@mastra/cli/cli/cli/workflows/candidate-workflow.ts"
...
To the existing `src/@@mastra/cli/cli/cli/workflows/candidate-workflow.ts` file add the following:
...
```ts filename="src/@@mastra/cli/cli/cli/workflows/candidate-workflow.ts" copy
...
To the existing `src/@@mastra/cli/cli/cli/workflows/candidate-workflow.ts` file change the `candidateWorkflow` like so:

```ts filename="src/@@mastra/cli/cli/cli/workflows/candidate-workflow.ts" copy {10-14}
export const candidateWorkflow = createWorkflow({
  id: "candidate-workflow",
  inputSchema: z.object({
    resumeText: z.string(),
...
In your `src/@@mastra/cli/cli/cli/index.ts` file, register the workflow:
...
const run = await @@mastra/cli/cli/cli.getWorkflow("candidateWorkflow").createRunAsync();
...
if (res.status === "success") {
  console.log(`Output value: ${res.result.question}`);
}
```
