...
### Install `@mastra/cli/cli/cli/mcp`

Add `@mastra/cli/cli/cli/mcp` to your project:

```bash copy
npm install @mastra/cli/cli/cli/mcp
```
...
```bash copy
rm -rf src/@@mastra/cli/cli/cli/agents src/@@mastra/cli/cli/cli/workflows src/@@mastra/cli/cli/cli/tools/weather-tool.ts
```

You should also change the `src/@@mastra/cli/cli/cli/index.ts` file like so:

```ts copy filename="src/@@mastra/cli/cli/cli/index.ts"
...
export const @@mastra/cli/cli/cli = new Mastra({
  storage: new LibSQLStore({
    // stores telemetry, evals, ... into memory storage, if it needs to persist, change to file:../@@mastra/cli/cli/cli.db
    url: ":memory:",
  }),
...
```bash copy
mkdir notes src/@@mastra/cli/cli/cli/mcp
```

Create the following files:

```bash copy
touch src/@@mastra/cli/cli/cli/mcp/{server,resources,prompts}.ts
```
...
In `src/@@mastra/cli/cli/cli/mcp/server.ts`, define the MCP server instance:

```typescript copy filename="src/@@mastra/cli/cli/cli/mcp/server.ts"
import { MCPServer } from "@mastra/cli/cli/cli/mcp";
...
Register this MCP server in your Mastra instance at `src/@@mastra/cli/cli/cli/index.ts`. The key `notes` is the public identifier for your MCP server:

```typescript copy filename="src/@@mastra/cli/cli/cli/index.ts" {4, 15-17}
...
Resource handlers allow clients to discover and read the content your server manages. Implement handlers to work with markdown files in the `notes` directory. Add to the `src/@@mastra/cli/cli/cli/mcp/resources.ts` file:

```typescript copy filename="src/@@mastra/cli/cli/cli/mcp/resources.ts"
import fs from "fs/promises";
import path from "path";
import { fileURLToPath } from "url";
import type { MCPServerResources, Resource } from "@mastra/cli/cli/cli/mcp";
...
Register these resource handlers in `src/@@mastra/cli/cli/cli/mcp/server.ts`:

```typescript copy filename="src/@@mastra/cli/cli/cli/mcp/server.ts" {2, 8}
import { MCPServer } from "@mastra/cli/cli/cli/mcp";
...
First, define the tool in `src/@@mastra/cli/cli/cli/tools/write-note.ts`:

```typescript copy filename="src/@@mastra/cli/cli/cli/tools/write-note.ts"
...
Register this tool in `src/@@mastra/cli/cli/cli/mcp/server.ts`:
...
Implement the prompts in `src/@@mastra/cli/cli/cli/mcp/prompts.ts`:

```typescript copy filename="src/@@mastra/cli/cli/cli/mcp/prompts.ts"
import type { MCPServerPrompts } from "@mastra/cli/cli/cli/mcp";
...
Register these prompt handlers in `src/@@mastra/cli/cli/cli/mcp/server.ts`:
