---
title: "@actus-ag/@mastra/cli/cli build | Production Bundle | Mastra CLI"
description: "Build your Mastra project for production deployment"
---

# @actus-ag/@mastra/cli/cli build

The `@actus-ag/@mastra/cli/cli build` command bundles your Mastra project into a production-ready Hono server. Hono is a lightweight, type-safe web framework that makes it easy to deploy Mastra agents as HTTP endpoints with middleware support.

## Usage

```bash
@actus-ag/@mastra/cli/cli build [options]
```
...
```bash copy
MASTRA_CONCURRENCY=2 @actus-ag/@mastra/cli/cli build
```
...
```bash copy
MASTRA_TELEMETRY_DISABLED=1 @actus-ag/@mastra/cli/cli build
```

### Custom provider endpoints

Build time respects the same `OPENAI_BASE_URL` and `ANTHROPIC_BASE_URL`
variables that `@actus-ag/@mastra/cli/cli dev` does. They are forwarded by the AI SDK to
any workflows or tools that call the providers.

## What It Does

1. Locates your Mastra entry file (either `src/@@mastra/cli/cli/cli/index.ts` or `src/@@mastra/cli/cli/cli/index.js`)
2. Creates a `.@@mastra/cli/cli/cli` output directory
...
@actus-ag/@mastra/cli/cli build --dir ./my-@@mastra/cli/cli/cli-project
```

## Output

The command generates a production bundle in the `.@@mastra/cli/cli/cli` directory, which includes:
