---
title: "@actus-ag/@mastra/cli/cli dev | Development Server | Mastra CLI"
description: Documentation for the @actus-ag/@mastra/cli/cli dev command, which starts a development server for agents, tools, and workflows.
---

# @actus-ag/@mastra/cli/cli dev

The `@actus-ag/@mastra/cli/cli dev` command starts a development server that exposes REST endpoints for your agents, tools, and workflows.

## Usage

```bash
@actus-ag/@mastra/cli/cli dev [options]
```
  ...
  content={[
    {
      name: "--dir",
      type: "string",
      description: "Path to your @@mastra/cli/cli/cli folder",
      isOptional: true,
    },
    {
      name: "--root",
...
Starting the server with `@actus-ag/@mastra/cli/cli dev` exposes a set of REST routes by default:
...
Agents are expected to be exported from `src/@@mastra/cli/cli/cli/agents`.
...
Tools are expected to be exported from `src/@@mastra/cli/cli/cli/tools` (or the configured tools directory).
...
Workflows are expected to be exported from `src/@@mastra/cli/cli/cli/workflows` (or the configured workflows directory).
...
The port defaults to 4111. Both the port and hostname can be configured via the @@mastra/cli/cli/cli server config. See [Launch Development Server](/docs/server-db/local-dev-playground) for configuration details.
...
To test an agent after running `@actus-ag/@mastra/cli/cli dev`:
...
The `@actus-ag/@mastra/cli/cli dev` server obeys a few extra environment variables that can
be handy during development:

### Disable build caching

Set `MASTRA_DEV_NO_CACHE=1` to force a full rebuild rather than using
the cached assets under `.@@mastra/cli/cli/cli/`:

```bash copy
MASTRA_DEV_NO_CACHE=1 @actus-ag/@mastra/cli/cli dev
```
...
```bash copy
MASTRA_CONCURRENCY=4 @actus-ag/@mastra/cli/cli dev
```
...
```bash copy
MASTRA_TELEMETRY_DISABLED=1 @actus-ag/@mastra/cli/cli dev
```
