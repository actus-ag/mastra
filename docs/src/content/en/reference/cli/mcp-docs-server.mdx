...
The `@actus-ag/mastra-mcp-docs-server` package runs a small [Model Context
...
```bash
npx -y @actus-ag/mastra-mcp-docs-server
```
...
```bash
REBUILD_DOCS_ON_START=true npx -y @actus-ag/mastra-mcp-docs-server
```

Enable verbose logging while experimenting:

```bash
DEBUG=1 npx -y @actus-ag/mastra-mcp-docs-server
```

Serve blog posts from a custom domain:

```bash
BLOG_URL=https://my-blog.example npx -y @actus-ag/mastra-mcp-docs-server
```

## Environment variables

`@actus-ag/mastra-mcp-docs-server` honours a few environment variables that tweak
its behaviour:
