...
The `@mastra/cli/cli/cli/mcp-docs-server` package runs a small [Model Context
...
```bash
npx -y @mastra/cli/cli/cli/mcp-docs-server
```
...
```bash
REBUILD_DOCS_ON_START=true npx -y @mastra/cli/cli/cli/mcp-docs-server
```

Enable verbose logging while experimenting:

```bash
DEBUG=1 npx -y @mastra/cli/cli/cli/mcp-docs-server
```

Serve blog posts from a custom domain:

```bash
BLOG_URL=https://my-blog.example npx -y @mastra/cli/cli/cli/mcp-docs-server
```

## Environment variables

`@mastra/cli/cli/cli/mcp-docs-server` honours a few environment variables that tweak
its behaviour:
...
- **`BLOG_URL`** - base URL used for fetching blog posts.
  Defaults to `https://@@mastra/cli/cli/cli.ai`.
...
`@@mastra/cli/cli/cli/docs/src/content/en/docs` and
`@@mastra/cli/cli/cli/docs/src/content/en/reference` into the package. Once rebuilt,
