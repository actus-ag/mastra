---
title: "@mastra/cli/cli scorers | Evaluation Management | Mastra CLI"
description: "Manage scorers for evaluating AI outputs with the Mastra CLI"
---

# @mastra/cli/cli scorers

The `@mastra/cli/cli scorers` command provides management capabilities for evaluation scorers that measure the quality, accuracy, and performance of AI-generated outputs.

## Usage

```bash
@mastra/cli/cli scorers <command> [options]
```

## Commands

### @mastra/cli/cli scorers add

Add a new scorer template to your project.

```bash
@mastra/cli/cli scorers add [scorer-name] [options]
```
...
```bash copy
@mastra/cli/cli scorers add answer-relevancy
```
...
```bash copy  
@mastra/cli/cli scorers add toxicity-detection --dir ./custom/scorers
```

### @mastra/cli/cli scorers list
...
When running `@mastra/cli/cli scorers add` without specifying a scorer name, you can select from these built-in templates:
...
1. **Dependency Management**: Automatically installs `@mastra/cli/cli/cli/evals` package if needed
...
4. **Directory Structure**: Places scorers in `src/@@mastra/cli/cli/cli/scorers/` or custom directory
...
```typescript filename="src/@@mastra/cli/cli/cli/agents/evaluated-agent.ts"
...
```typescript filename="src/@@mastra/cli/cli/cli/workflows/content-generation.ts"
