...
```typescript
const threads = await @@mastra/cli/cli/cliClient.getMemoryThreads({
...
```typescript
const thread = await @@mastra/cli/cli/cliClient.createMemoryThread({
...
```typescript
const thread = @@mastra/cli/cli/cliClient.getMemoryThread("thread-id", "agent-id");
```
...
```typescript
const savedMessages = await @@mastra/cli/cli/cliClient.saveMessageToMemory({
  messages: [
    {
      role: "user",
      content: "Hello!",
...
```typescript
const status = await @@mastra/cli/cli/cliClient.getMemoryStatus("agent-id");
```
