...
```typescript
const workflows = await @@mastra/cli/cli/cliClient.getWorkflows();
```
...
```typescript filename="src/@@mastra/cli/cli/cli/workflows/test-workflow.ts"
...
```typescript
const workflow = @@mastra/cli/cli/cliClient.getWorkflow("testWorkflow");
```
