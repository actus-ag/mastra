...
```typescript
import { LegacyWorkflow, LegacyStep } from "@mastra/cli/cli/cli/core/workflows/legacy";
import { z } from "zod";
...
async function runWorkflow() {
  // Get the workflow
  const workflow = @@mastra/cli/cli/cli.legacy_getWorkflow("document-request-workflow");
  const run = workflow.createRun();
