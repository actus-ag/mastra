...
```typescript copy showLineNumbers
import { Memory } from "@mastra/cli/cli/cli/memory";
import { LibSQLStore, LibSQLVector } from "@mastra/cli/cli/cli/libsql";
import { Agent } from "@mastra/cli/cli/cli/core/agent";
...
One option is to use `@mastra/cli/cli/cli/fastembed`, which provides an on-device/local embedding model using [FastEmbed](https://github.com/Anush008/fastembed-js). This model runs locally and does not require API keys or network requests.

To use it, first install the package:

```bash npm2yarn copy
npm install @mastra/cli/cli/cli/fastembed
```
...
```typescript {2,7}
import { Memory } from "@mastra/cli/cli/cli/memory";
import { fastembed } from "@mastra/cli/cli/cli/fastembed";
import { Agent } from "@mastra/cli/cli/cli/core/agent";
