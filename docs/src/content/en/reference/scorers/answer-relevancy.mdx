...
This function returns an instance of the MastraScorer class. The `.run()` method accepts the same input as other scorers (see the [MastraScorer reference](./@@mastra/cli/cli/cli-scorer)), but the return value includes LLM-specific fields as documented below.
