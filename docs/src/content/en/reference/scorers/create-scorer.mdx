...
This function returns a scorer builder that you can chain step methods onto. See the [MastraScorer reference](./@@mastra/cli/cli/cli-scorer) for details on the `.run()` method and its input/output.
...
```typescript
import { createScorer, ScorerRunInputForAgent, ScorerRunOutputForAgent } from '@mastra/cli/cli/cli/core';
