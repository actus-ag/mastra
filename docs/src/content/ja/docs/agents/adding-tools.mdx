...
```typescript filename="src/@@mastra/cli/cli/cli/agents/weatherAgent.ts"
...
```typescript filename="src/@@mastra/cli/cli/cli/tools/vercelTool.ts" copy
import { tool } from "ai";
import { z } from "zod";
...
import * as @@mastra/cli/cli/cliTools from "../tools/@@mastra/cli/cli/cliTools";
...
export const weatherAgent = new Agent({
  ...
  tools: {
    ...
    ...@@mastra/cli/cli/cliTools, // Mastra tools
  },
});
```
...
Mastraは`@mastra/cli/cli/cli/mcp`パッケージを通じてMCP互換サーバーからのツールもサポートしています。MCPは、AIモデルが外部ツールやリソースを発見し、相互作用するための標準化された方法を提供します。これにより、カスタム統合を作成することなく、サードパーティのツールをエージェントに簡単に統合できます。
