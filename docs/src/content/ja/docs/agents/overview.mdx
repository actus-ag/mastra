...
```ts showLineNumbers filename="src/@@mastra/cli/cli/cli/agents/index.ts" copy
...
また、`@mastra/cli/cli/cli/core`パッケージがインストールされていることを確認してください：

```bash npm2yarn copy
npm install @mastra/cli/cli/cli/core@latest
```
...
Mastraは、エージェントをAPIの背後で実行するための`@actus-ag/@mastra/cli/cli dev`というCLIコマンドを提供しています。デフォルトでは、`src/@@mastra/cli/cli/cli/agents`ディレクトリ内のファイルでエクスポートされたエージェントを探します。
