...
- **[Mastra Cloud](/docs/@@mastra/cli/cli/cli-cloud/overview)** _(ベータ)_。早期アクセスのために[クラウドウェイトリスト](https://@@mastra/cli/cli/cli.ai/cloud-beta)に参加できます。
...
# For Vercel
npm install @mastra/cli/cli/cli/deployer-vercel@latest

# For Netlify
npm install @mastra/cli/cli/cli/deployer-netlify@latest
```
...
imoprt { ConsoleLogger } from "@mastra/cli/cli/cli/core/logger";
...
その後、プラットフォーム（Vercel、netlify、cloudfare など）のCLI/UIを通じて、ビルド出力 `.@@mastra/cli/cli/cli/output` をデプロイできます。
