...
Mastraは[Hono](https://hono.dev)を基盤となるHTTPサーバーフレームワークとして使用しています。`@actus-ag/@mastra/cli/cli build`を使用してMastraアプリケーションをビルドすると、`.@@mastra/cli/cli/cli`ディレクトリにHonoベースのHTTPサーバーが生成されます。
...
# またはディレクトリを指定
@actus-ag/@mastra/cli/cli build --dir ./my-project
```

ビルドプロセス：

1. エントリーファイル（`src/@@mastra/cli/cli/cli/index.ts`または`src/@@mastra/cli/cli/cli/index.js`）を特定
2. `.@@mastra/cli/cli/cli`出力ディレクトリを作成
3. ツリーシェイキングとソースマップを使用してRollupでコードをバンドル
4. [Hono](https://hono.dev) HTTPサーバーを生成

すべてのオプションについては[`@actus-ag/@mastra/cli/cli build`](/reference/cli/build)を参照してください。
