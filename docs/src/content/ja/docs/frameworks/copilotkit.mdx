...
<Tabs items={["npm", "yarn", "pnpm"]}>
  <Tabs.Tab>```bash copy npm install @datawarp/mastra-client-js ```</Tabs.Tab>
  <Tabs.Tab>```bash copy yarn add @datawarp/mastra-client-js ```</Tabs.Tab>
  <Tabs.Tab>```bash copy pnpm add @datawarp/mastra-client-js ```</Tabs.Tab>
</Tabs>
