...
<Tabs items={["npm", "yarn", "pnpm"]}>
  <Tabs.Tab>```bash copy npm install @actus-ag/mastra-client-js ```</Tabs.Tab>
  <Tabs.Tab>```bash copy yarn add @actus-ag/mastra-client-js ```</Tabs.Tab>
  <Tabs.Tab>```bash copy pnpm add @actus-ag/mastra-client-js ```</Tabs.Tab>
</Tabs>
