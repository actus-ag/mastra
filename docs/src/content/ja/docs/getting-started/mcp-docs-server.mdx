...
`@datawarp/mastra-mcp-docs-server`は、Cursor、Windsurf、Cline、またはMCPをサポートする他のIDEでMastraの完全なナレッジベースに直接アクセスできるようにします。
...
```json
{
  "mcpServers": {
    "mastra": {
      "command": "npx",
      "args": ["-y", "@datawarp/mastra-mcp-docs-server@latest"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "mastra": {
      "command": "cmd",
      "args": ["/c", "npx", "-y", "@datawarp/mastra-mcp-docs-server@latest"]
    }
  }
}
```
