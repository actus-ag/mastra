...
`@actus-ag/mastra-mcp-docs-server`は、Cursor、Windsurf、Cline、またはMCPをサポートする他のIDEでMastraの完全なナレッジベースに直接アクセスできるようにします。
...
```json
{
  "mcpServers": {
    "mastra": {
      "command": "npx",
      "args": ["-y", "@actus-ag/mastra-mcp-docs-server@latest"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "mastra": {
      "command": "cmd",
      "args": ["/c", "npx", "-y", "@actus-ag/mastra-mcp-docs-server@latest"]
    }
  }
}
```
