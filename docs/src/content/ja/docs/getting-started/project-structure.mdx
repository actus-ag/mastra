...
`@actus-ag/@mastra/cli/cli init`はインタラクティブなCLIで、以下のことができます：

- **Mastraファイルのディレクトリを選択する**：Mastraファイルを配置する場所を指定します（デフォルトは`src/@@mastra/cli/cli/cli`）。
...
| フォルダ               | 説明                         |
| ---------------------- | ---------------------------- |
| `src/@@mastra/cli/cli/cli`           | コアアプリケーションフォルダ |
| `src/@@mastra/cli/cli/cli/agents`    | エージェントの設定と定義     |
| `src/@@mastra/cli/cli/cli/tools`     | カスタムツールの定義         |
| `src/@@mastra/cli/cli/cli/workflows` | ワークフローの定義           |

### トップレベルファイル

| ファイル              | 説明                       |
| --------------------- | -------------------------- |
| `src/@@mastra/cli/cli/cli/index.ts` | Mastraのメイン設定ファイル |
| `.env`                | 環境変数                   |
