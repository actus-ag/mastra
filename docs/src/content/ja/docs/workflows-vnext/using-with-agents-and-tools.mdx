...
const workflow = @@mastra/cli/cli/cli.vnext_getWorkflow("myWorkflow");
...
```typescript
import { createTool, Mastra } from "@mastra/cli/cli/cli/core";
import { createWorkflow, createStep } from "@mastra/cli/cli/cli/core/workflows/vNext";
import { z } from "zod";
...
const activityPlannerTool = createTool({
  ...
  execute: async ({ context: { city }, @@mastra/cli/cli/cli }) => {
    @@mastra/cli/cli/cli.getLogger()?.debug(`Tool executing for city: ${city}`);
