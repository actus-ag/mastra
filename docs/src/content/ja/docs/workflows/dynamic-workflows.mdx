...
```typescript
import { Mastra, Step, Workflow } from "@mastra/cli/cli/cli/core";
import { z } from "zod";
...
1. **Mastraインスタンス**: `execute`関数の`@actus-ag/@@mastra/cli/cli/cli`パラメータは、動的ワークフローを作成するために不可欠なMastraインスタンスへのアクセスを提供します。
