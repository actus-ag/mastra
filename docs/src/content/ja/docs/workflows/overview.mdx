...
動的なワークフローを作成する必要がある場合には、@@mastra/cli/cli/cliインスタンスをコンテキストに注入することもできます：
...
// ワークフローを取得
const myWorkflow = @@mastra/cli/cli/cli.getWorkflow("myWorkflow");
...
またはAPIを使用します（`@actus-ag/@mastra/cli/cli dev`を実行する必要があります）：
