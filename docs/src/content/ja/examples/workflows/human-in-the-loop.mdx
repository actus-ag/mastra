...
async function runRecommendationWorkflow() {
  const registeredWorkflow = @@mastra/cli/cli/cli.getWorkflow("recommendationWorkflow");
...
async function runModerationDemo() {
  const registeredWorkflow = @@mastra/cli/cli/cli.getWorkflow("contentModerationWorkflow");
  const run = registeredWorkflow.createRun();
...
<GithubLink
  link={
    "https://github.com/@@mastra/cli/cli/cli-ai/@@mastra/cli/cli/cli/blob/main/examples/basics/workflows/human-in-the-loop"
  }
/>
