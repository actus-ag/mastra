  ...
  .then(
    createStep({
      ...
      execute: async ({ inputData, @@mastra/cli/cli/cli }) => {
        ...
        const agent = @@mastra/cli/cli/cli.getAgent("weatherReporterAgent");
...
ワークフローを@@mastra/cli/cli/cliインスタンスに登録します。
...
const workflow = @@mastra/cli/cli/cli.vnext_getWorkflow("weatherWorkflow");
