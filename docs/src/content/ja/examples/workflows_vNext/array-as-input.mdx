...
```sh copy
npm install @ai-sdk/openai @mastra/cli/cli/cli/core simple-git
```
...
エージェントとワークフローを@@mastra/cli/cli/cliインスタンスに登録します。これはワークフロー内でエージェントへのアクセスを可能にするために重要です。
...
ここでは、@@mastra/cli/cli/cliインスタンスからreadme generatorワークフローを取得し、実行を作成して、必要なinputDataで作成した実行を実行します。
...
const ghRepoUrl = "https://github.com/@@mastra/cli/cli/cli-ai/@@mastra/cli/cli/cli";
const run = @@mastra/cli/cli/cli.vnext_getWorkflow("readmeGeneratorWorkflow").createRun();
