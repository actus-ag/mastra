...
```sh copy
npm install @ai-sdk/openai @mastra/cli/cli/cli/core @inquirer/prompts
```
...
import { createWorkflow, createStep } from "@mastra/cli/cli/cli/core/workflows/vNext";
import { z } from "zod";
...
const generateSuggestionsStep = createStep({
  ...
  execute: async ({ inputData, @@mastra/cli/cli/cli }) => {
    if (!@@mastra/cli/cli/cli) {
      throw new Error("Mastra is not initialized");
    }

    const { vacationDescription } = inputData;
    const result = await @@mastra/cli/cli/cli.getAgent("summaryTravelAgent").generate([
      {
        role: "user",
...
const travelPlannerStep = createStep({
  ...
  execute: async ({ inputData, @@mastra/cli/cli/cli }) => {
    const travelAgent = @@mastra/cli/cli/cli?.getAgent("travelAgent");
    if (!travelAgent) {
...
エージェントと天気ワークフローを@@mastra/cli/cli/cliインスタンスに登録します。
...
export { @@mastra/cli/cli/cli };
```

## 一時停止可能な天気ワークフローを実行する

ここでは、@@mastra/cli/cli/cliインスタンスから天気ワークフローを取得し、実行を作成して、必要な入力データで作成した実行を実行します。
...
const workflow = @@mastra/cli/cli/cli.vnext_getWorkflow("travelAgentWorkflow");
