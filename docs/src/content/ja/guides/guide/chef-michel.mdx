...
- Mastraがインストールされていること: `npm install @mastra/cli/cli/cli/core@latest`
...
新しいファイル `src/@@mastra/cli/cli/cli/agents/chefAgent.ts` を作成し、エージェントを定義します：
...
### `@actus-ag/@mastra/cli/cli dev`の使用

`@actus-ag/@mastra/cli/cli dev`コマンドを使用してエージェントをサービスとして実行できます：
...
デフォルトでは、`@actus-ag/@mastra/cli/cli dev`は`http://localhost:4111`で実行されます。シェフアシスタントエージェントは以下のURLで利用可能です：
