---
title: "`@actus-ag/@mastra/cli/cli dev` リファレンス | ローカル開発 | Mastra CLI"
description: エージェント、ツール、ワークフローの開発サーバーを起動する@actus-ag/@mastra/cli/cli devコマンドのドキュメント。
---

# `@actus-ag/@mastra/cli/cli dev` リファレンス

`@actus-ag/@mastra/cli/cli dev` コマンドは、エージェント、ツール、ワークフローのRESTルートを公開する開発サーバーを起動します。
...
`@actus-ag/@mastra/cli/cli dev`でサーバーを起動すると、デフォルトで一連のRESTルートが公開されます。
...
エージェントは`src/@@mastra/cli/cli/cli/agents`からエクスポートされている必要があります。
...
ツールは`src/@@mastra/cli/cli/cli/tools`（または設定されたツールディレクトリ）からエクスポートされている必要があります。
...
ワークフローは`src/@@mastra/cli/cli/cli/workflows`（または設定されたワークフローディレクトリ）からエクスポートされている必要があります。
...
ポートのデフォルトは4111です。ポートとホスト名の両方は@@mastra/cli/cli/cliサーバー設定で構成できます。設定の詳細については[開発サーバーの起動](/docs/local-dev/@@mastra/cli/cli/cli-dev#launch-development-server)を参照してください。
...
`@actus-ag/@mastra/cli/cli dev`を実行した後にエージェントをテストするには：
...
`@actus-ag/@mastra/cli/cli dev`サーバーは、開発中に便利ないくつかの追加環境変数に従います：

### ビルドキャッシュの無効化

`MASTRA_DEV_NO_CACHE=1`を設定すると、`.@@mastra/cli/cli/cli/`以下のキャッシュされたアセットを使用する代わりに、完全な再ビルドを強制します：
