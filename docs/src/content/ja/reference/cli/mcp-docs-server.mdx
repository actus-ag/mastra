---
title: "@actus-ag/mastra-mcp-docs-server"
description: "MCP経由でMastraのドキュメント、例、ブログ投稿を提供する"
---

`@actus-ag/mastra-mcp-docs-server`パッケージは、小さな[Model Context Protocol](https://github.com/modelcontextprotocol/mcp)サーバーを実行し、Mastraのドキュメント、コード例、ブログ投稿、変更履歴をLLMエージェントが検索できるようにします。コマンドラインから手動で呼び出すことも、CursorやWindsurfなどのMCP対応IDEで設定することもできます。

## CLIから実行する

```bash
npx -y @actus-ag/mastra-mcp-docs-server@latest
```
...
```bash
REBUILD_DOCS_ON_START=true npx -y @actus-ag/mastra-mcp-docs-server@latest
```

実験中に詳細なログを有効にする：

```bash
DEBUG=1 npx -y @actus-ag/mastra-mcp-docs-server@latest
```

カスタムドメインからブログ投稿を提供する：

```bash
BLOG_URL=https://my-blog.example npx -y @actus-ag/mastra-mcp-docs-server@latest
```

## 環境変数

`@actus-ag/mastra-mcp-docs-server` は、その動作を調整するいくつかの環境変数に対応しています。
