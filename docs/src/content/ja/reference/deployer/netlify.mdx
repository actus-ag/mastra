...
```bash
npx @actus-ag/@mastra/cli/cli lint
```
...
ビルドプロセスにより、`.@@mastra/cli/cli/cli/output`ディレクトリに次のような出力構造が生成されます。

```
.@@mastra/cli/cli/cli/output/
├── netlify/
│   └── functions/
│       └── api/
...
NetlifyDeployerは、`.@@mastra/cli/cli/cli/output`内に自動的に`netlify.toml`設定ファイルを次の内容で生成します。
...
ビルド後、Mastraアプリケーションの `.@@mastra/cli/cli/cli/output` をNetlifyに以下のいずれかの方法でデプロイできます：

1. **Netlify CLI**: Netlifyの公式CLIツールを使って直接デプロイ

   - CLIをインストール: `npm install -g netlify-cli`
   - 出力ディレクトリに移動: `cd .@@mastra/cli/cli/cli/output`
...
> 出力ディレクトリ `.@@mastra/cli/cli/cli/output` で `netlify dev` を実行することで、Mastraアプリケーションをローカルでテストすることもできます。
