...
ビルドプロセスは`.@@mastra/cli/cli/cli/output`ディレクトリに以下の出力構造を生成します：
...
VercelDeployerは自動的に`.@@mastra/cli/cli/cli/output`に以下の設定を含む`vercel.json`設定ファイルを生成します：
...
ビルド後、Mastraアプリケーションの `.@@mastra/cli/cli/cli/output` を以下のいずれかの方法でVercelにデプロイできます：
...
> 出力ディレクトリ `.@@mastra/cli/cli/cli/output` で `vercel dev` を実行することで、Mastraアプリケーションをローカルでテストすることもできます。
