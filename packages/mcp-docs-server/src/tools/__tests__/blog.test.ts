...
describe('blog tool', () => {
  ...
  test('fetches and parses blog posts correctly', async () => {
    const result = await callTool(tools.@mastra_@mastraBlog, { url: '/api/blog' });
  ...
  test('handles fetch errors gracefully', async () => {
    const result = await callTool(tools.@mastra_@mastraBlog, { url: `${baseUrl}/api/blog/non-existent-post` });
  ...
  test('returns specific blog post content when URL is provided', async () => {
    const result = await callTool(tools.@mastra_@mastraBlog, { url: `${baseUrl}/api/blog/principles-of-ai-engineering` });
  ...
  test('handles empty blog post content', async () => {
    const result = await callTool(tools.@mastra_@mastraBlog, { url: `${baseUrl}/api/blog/empty-post` });
