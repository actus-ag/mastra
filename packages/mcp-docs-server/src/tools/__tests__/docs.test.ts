...
describe('docsTool', () => {
  ...
  describe('execute', () => {
    it('should list directory contents when no specific path is requested', async () => {
      const result = await callTool(tools.@mastra_@mastraDocs, { paths: [''] });
    ...
    it('should return content for index.mdx', async () => {
      const result = await callTool(tools.@mastra_@mastraDocs, { paths: ['index.mdx'] });
      expect(result).toContain('## index.mdx');
      expect(result).toContain('# About Mastra');
    });

    it('should handle directory listings', async () => {
      const result = await callTool(tools.@mastra_@mastraDocs, { paths: ['reference'] });
    ...
    it('should handle non-existent paths gracefully', async () => {
      const result = await callTool(tools.@mastra_@mastraDocs, { paths: ['non-existent-path'] });
    ...
    it('should handle multiple paths in a single request', async () => {
      const result = await callTool(tools.@mastra_@mastraDocs, {
    ...
    it('should find nearest directory when path is partially correct', async () => {
      const result = await callTool(tools.@mastra_@mastraDocs, { paths: ['reference/tools/non-existent'] });
    ...
    it('should handle paths with special characters', async () => {
      const result = await callTool(tools.@mastra_@mastraDocs, { paths: ['reference/tools/'] });
      expect(result).toContain('Directory contents of reference/tools');
    });

    it('should handle MDX files with frontmatter', async () => {
      const result = await callTool(tools.@mastra_@mastraDocs, { paths: ['reference/rag/document.mdx'] });
    ...
    it('should work when queryKeywords is an empty array', async () => {
      const result = await callTool(tools.@mastra_@mastraDocs, { paths: ['reference'], queryKeywords: [] });
      expect(result).toContain('Directory contents of reference');
    });
    ...
    it('should return directory contents when given a valid path', async () => {
      const result = await callTool(tools.@mastra_@mastraDocs, { paths: ['reference'], queryKeywords: ['rag'] });
      expect(result).toContain('Directory contents of reference');
    });
